...
//    wave_save_snapshot(s, shot, u0, 0%s->nb_snap==0);
    ...
    // save snapshot of domain at end of computation
    MSG("SNAPSHOT FINAL");
    FILE *snap_fd;
    char *snap_fd_name = (char*)malloc(20*sizeof(char));

    sprintf(snap_fd_name, "snapshot_final");
    snap_fd = fopen(snap_fd_name, "wb+");

    CHK(snap_fd == NULL, "failed to open snapshot final file");
    CHK(fwrite(u1, sizeof(float), s->size, snap_fd) != s->size, "failed to write snapshot final");
    CHK(fclose(snap_fd) != 0, "failed to close snapshot final file");
    if (s->verbose) MSG("... saving snapshot final (size %d)", s->size);
    free(snap_fd_name);
...
//    wave_tb_data_set_rcv(data,s,sismos);
    ...
    MSG("ˇˇ COMPUTATION MESSAGES SHOULD BE HERE ˇˇ");
    wave_tb_forward(ctx,data,timer,u0,u1,vel);
    MSG("^^ COMPUTATION MESSAGES SHOULD BE HERE ^^");
 ...
 //   wave_tb_data_unset_rcv(data);
    ...
    // save snapshot of domain at end of computation
    wave_tb_save_lastshot(s, shot, u0, u1, 104); //last value t=? is not used in function body

    /// save the seismic traces for the shot.
    // @MODIF louis : commented because it writes a file of 0s, need to add sismos_extract in wave_tb code
    //wave_save_sismos(s, shot, sismos);
...
int main(int argc, char* argv[]) {
  ...
  MSG("start wave_init_numerics0");
  MSG("... cpu=: %d\n", s->cpu);
  MSG("start wave_init_numerics");
  wave_init_numerics(s);
  MSG("wave_init_numerics");
  /// initialize the velocity and the compute sizes.
  wave_init_dimensions(s);
  MSG("wave_init_dimensions");
  wave_init_damp(s);
  MSG("wave_init_damp");
