...
export TIME_TB_2nd=528; #@pavel in TB source injection starts from second time sample (Nothing happens for one dt).This is code feature.
export TIME_SB_2nd=527; #@pavel in SB the nt should one time less than in correponding TB.
#export TIME_TB_2nd=240; #@pavel in TB source injection starts from second time sample (Nothing happens for one dt).This is code feature.
#export TIME_SB_2nd=239; #@pavel in SB the nt should one time less than in correponding TB.

dt=0.001;
shot=16447;  # position of the source in x,y coordinates.check ./data/acquisition.txt
shot=32639;  # position of the source in x,y coordinates.check ./data/acquisition.txt
shot=32712;  # position of the source in x,y coordinates.check ./data/acquisition.txt
src_depth=256;
x=2; y=2; z=1; t=7; w=20; tgs=4;
export OMP_NUM_THREADS=4

./bin/modeling --verbose --n1 $nx  --n2 $ny --n3 $nz --iter $TIME_SB_2nd \
--mode 2 --drcv 1 --dshot 1 --first $shot --last $shot --src_depth 256 --order 2 --fmax 8;
...
# ./scripts_useful/diff_to ./snapshot_TB2nd_528 ./snapshot_SB2nd_527