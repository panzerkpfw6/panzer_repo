...
# Set compiler flags for best performance with icx/icpx (2024.1)
export CFLAGS="-xHost -qopenmp -O3 -ipo -funroll-loops -qopt-streaming-stores=always -qopt-mem-layout-trans=4"
export CXXFLAGS="-xHost -qopenmp -O3 -ipo -funroll-loops -qopt-streaming-stores=always -qopt-mem-layout-trans=4"
export FFLAGS="-xHost -qopenmp -O3 -ipo -funroll-loops -qopt-streaming-stores=always -qopt-mem-layout-trans=4"

###********** MODULES *********###
# Commenting out module load to use explicit local compiler path
#module load intel-oneapi-compilers/2022.2.1/gcc-11.3.0-k2f52ij
source ~/.bashrc
source /media/plotnips/sdd1/soft/intel/oneapi/2024.1/setvars.sh
module load cmake  # Keep cmake module as it’s still needed

###***********************###
# Explicitly set the Intel oneAPI compiler paths (local installation)
...
##### COMPILATION #####
rm -f ./bin/modeling  # Remove old executable if it exists
rm -f CMakeCache.txt  # Clean CMake cache
rm -rf CMakeFiles     # Remove CMakeFiles directory
cmake -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" .
make clean
# Ensure library is indexed before linking
#[ -f ../src/libstencil.a ] && ranlib ../src/libstencil.a
make VERBOSE=1
make install
...
# Optional: Profile with gprof to match <DOCUMENT>
# export CFLAGS="$CFLAGS -g -pg"
# export CXXFLAGS="$CXXFLAGS -g -pg"
# export FFLAGS="$FFLAGS -g -pg"
# cmake -DCMAKE_C_COMPILER="$CC" -DCMAKE_CXX_COMPILER="$CXX" .
# make clean && [ -f ../src/libstencil.a ] && ranlib ../src/libstencil.a && make VERBOSE=1 && make install
# ./bin/modeling --verbose --n1 $nx --n2 $ny --n3 $nz --iter $NT_SB_1st \
# --mode 2 --drcv 1 --dshot 1 --first $shot --last $shot --src_depth $src_depth --order 1 --fmax $fmax \
# --dx $dx >> $logs_path/log-SB_1st-abc_$grid_str.log
