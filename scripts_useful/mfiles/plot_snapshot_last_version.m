...
fname=['../../stencil-sismos/raws/SB_1st_abc.raw']
% fname=['../../stencil-sismos/SB_abc/data/used_vel_model.raw']
dims=[512,512,512];
dims=[676,676,201];
% dims=[200,200,200];
% dims=[512,512,300];
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
snap=squeeze(data( floor(dims(1)/2),:,:)).';
snap=snap./max(abs(snap),[],'all');
slice_yz=snap;
snap=squeeze(data(:,floor(dims(2)/2),:)).';
snap=snap./max(abs(snap),[],'all');
slice_xz=snap;
snap=squeeze(data(:,:,5 )).';
% snap=squeeze(data(:,:,floor(dims(3)/2) ) ).';
snap=snap./max(abs(snap),[],'all');
slice_xy_100=snap;
...
set(colorTitleHandle ,'String',titleString,'FontSize',font_sz);
set(gca,'FontSize',font_sz,'fontWeight','bold');
% name='/home/plotnips/Dropbox/Apps/Overleaf/PASC_2024_stencil/figures/snapshots_sb_abc.png'; delete(name); print(name,'-dpng',['-r' num2str(400)]);
% name='/home/plotnips/Dropbox/Apps/Overleaf/PASC_2024_stencil/figures/snapshots_sb_abc.pdf'; delete(name); print(name,'-dpdf',['-r' num2str(400)]);
ss=1;
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
min% [fe,ve,ce] = isocaps(x,y,z,D,5);               
% figure
