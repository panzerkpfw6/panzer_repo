...
# !pwd
# !unzip ./grid_perf/emerald_rapids.zip -d ./grid_perf/
# !unzip ./grid_perf/Shaheen3_genoa_results.zip -d ./grid_perf/
...
OMP_NUM_THREADS=max(data['cores'].unique())
...
df_scalability_plot=df_scalability_plot.sort_values(by=['SB'],ascending=False)
sys='Emerald'
##################################################################
...
tb_source='TB'
labels={
        'Emerald-SB':'SB, 1st order, Emerald',
        'Emerald-TB':'MWD, 1st order, Emerald',
        }
colors=['blue','orange']
fig=plt.figure(figsize=(7,2.0))
for icolor,code in enumerate(['TB', 'SB']):
    if code=='SB':   
        yval=df_scalability_plot['SB'];
        sb=yval.array[0];
    if code=='TB':   
        yval=df_scalability_plot['TB'];
        tb=yval.array[0];
...
tb_source='TB_order2'
labels={
        'Emerald-SB':'SB, 2nd order, Emerald',
        'Emerald-TB':'MWD, 2nd order, Emerald',
        }
colors=['blue','orange']
fig=plt.figure(figsize=(7,2.0))
for icolor,code in enumerate(['TB', 'SB']):
