...
list_of_folders=['/Users/pavelplotnitskii/Library/CloudStorage/Dropbox/PhD_proposal/work_with_david/emerald_rapids']
sim_info=[]
times=[]
giga_points=[]
giga_flops = []
...
arch_name=list_of_folders[0].split('/')[-1]
data_summary=data.copy()
data_summary=data_summary.drop(columns=['giga_flop_s','point_updates','Flops','sim_info','times'])
data_summary.to_excel(os.path.join('data_summary_whole_data_'+arch_name+'.xlsx'))
...
data_tmp = pd.DataFrame({
    ...
    'giga_point_s':[],
    'cores':[]})
OMP_NUM_THREADS=data['cores'].unique()[0]

for grid_ in grids_:
    data_grid=data_summary[data_summary['grids']==grid_]
    data_grid=data_grid[data_grid['cores']==OMP_NUM_THREADS]
...
df_scalability_plot=df_scalability_plot.sort_values(by=['num_th'],ascending=True)
y_max=50
sys='Emerald'
...
tb_source='TB'
labels={
        sys+'-SB':'SB, 1st order, '+sys,
        sys+'-TB':'MWD, 1st order, '+sys,
        }
colors=['blue','orange']
fig=plt.figure(figsize=(7,2.0))
for icolor,code in enumerate(['TB', 'SB']):
    if code=='SB':   
        yval=df_scalability_plot[sb_source];
    if code=='TB':   
        yval=df_scalability_plot[tb_source];
...
# algo speedup
yval=df_scalability_plot[sb_source];sb=yval.array[-1];
yval=df_scalability_plot[tb_source];tb=yval.array[-1];
core_x=df_scalability_plot['num_th'].array[-1]

line1=ax.arrow(core_x,sb,0,(tb-sb), color='g', width=0.40, length_includes_head=True, head_length=1)
ax.annotate(str(np.round(tb/sb,1))+"X", xy=(core_x, (sb+tb)/2), xytext=(core_x-core_x/10,(sb+tb)/2-0.9), color='g', fontsize=14)
# plt.show()
...
sb_source='SB_order2'
tb_source='TB_order2'
labels={ sys+'-SB':'SB, 2nd order, '+sys,
        sys+'-TB':'MWD, 2nd order, '+sys,}
colors=['blue','orange']
fig=plt.figure(figsize=(7,2.0))
for icolor,code in enumerate(['TB', 'SB']):
    if code=='SB':   
